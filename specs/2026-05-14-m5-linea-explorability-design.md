...
- 点击命中：g.arc-hit-layer 透明 16 屏幕 px stroke overlay（DR-061 / 解 PM "太细难选中"）
  · **R3 修 DR-067 命中歧义**：hit click 用 pickNearestArc(clientX, clientY) 几何最近择优 / elementsFromPoint 拿所有 candidates + 32 点采样最短距离² / 不靠 DOM stacking 顺序
  · **R4 修 DR-068 hover preview 根治误选**：R3 仍误选（几何最近 ≠ 视觉认知最近）/ arc-hit-layer 加 mousemove + RAF 节流 + pickNearestArc 实时高亮"将命中弧"/ click 前确认 / preview-then-commit 跟 Stage 4 + R2 焦点按钮一致 / mouseleave 复原（selected 弧保留高亮）
