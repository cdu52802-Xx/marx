...
| DR-088 | 2026-05-21 | B1 DR-079 polish 落地方案 A · search popover 180ms `fade + translateY(-4px)` ease-out 入场 + 全局 `prefers-reduced-motion` 兜底 | B 段头 staggered reveal（+10 行 JS / 视觉差不可感）/ C 砍掉 polish（A- + AI Slop A 已超阀值）/ D mockup 先看（motion 静态 mockup 无法体现）| (1) PM 反馈 4 维度（字体/间距/配色/微动效）中 3 个 styles.css audit 已到位 / 真改空间集中"微动效"；(2) 单整体 fade+translate 比 staggered 投入产出比高（B 视觉差用户感知不出 / 复杂度↑）；(3) GPU 合成 transform+opacity / 不破 layout / 不触发 reflow；(4) reduced-motion 全局兜底顺手修 Finding 3 / 尊重晕动症 + 视觉敏感用户（WCAG 2.3.3）；(5) Bundle gzip JS 持平 34.42 KB / CSS +0.14 KB（远不破 ≤35 预算）；(6) PM 拍板"直接实施 + 浏览器实测"不出 mockup（motion 静态图无效）|
