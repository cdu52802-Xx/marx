...
> **状态**：M3 进行中 · Task 1-7 完成（person 4 字段全 AI 草稿入库）· Task 8 待启动（work 全字段）
...
| **M3（数据采集阶段 B 校对 + C 后来者旁注）** | **🔄 进行中（Task 8/17，9-17 待办）** | 见下表 |
...
| [`docs/m3-validation/work-checklist.md`](../docs/m3-validation/work-checklist.md) | 4 work · 全字段 | Task 2 | 🟡 全占位，Task 8 才填 |
| `docs/m3-validation/concept-checklist.md` | 12 concept · 全字段 | Task 9 创建 | ⏳ 未创建 |
...
## Task 4-7 残留待复核（opportunistic 模式延后）
...
- ✅ 跑 `npm run m3:apply-person` 影响 30 个节点
- ✅ 跑 `npm test` 维持 21 tests / 18 PASS / 3 FAIL（错误信息从 citation 移到 name_orig：Q136116320 Alfred Herman name_orig === name_zh 触发 line 32 fail，因为 M2 SPARQL fallback；3 不确定节点 4 字段全空是 expected RED）
- ✅ commit + 不 push（Task 7 完成 + 累计 4 commit 一起 push）
- ⏭️ Task 8 待启动（4 work 节点 5 字段，决策 4 默认走 AI 草稿直接 apply 模式）
...
当前位置：Task 4-7 已完成（person 4 字段全 AI 草稿入库），Task 8 待启动（work 全字段）。

Task 8+ 默认走决策 4 模式：AI 出草稿 → 直接 apply → 待 opportunistic 复核。
