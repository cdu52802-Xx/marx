...
> **状态**：M3 进行中 · Task 1-5 完成（决策 4 opportunistic 模式扩到 latlng）· Task 6 待启动（bio）
...
| **M3（数据采集阶段 B 校对 + C 后来者旁注）** | **🔄 进行中（Task 6/17，7-17 待办）** | 见下表 |
...
## Task 4-5 残留待复核（opportunistic 模式延后）
...
### 🟢 23 个高把握节点 · name_orig + latlng 都已入库 + 不在学术敏感清单
...
- ✅ 跑 `npm test` 维持 21 tests / 18 PASS / 3 FAIL（错误信息从 latlng 移到 bio：Q9235 黑格尔 bio 缺失等 Task 6 解锁）
- ✅ commit + 不 push（等 Task 6/7 完成一起 push）
- ⏭️ Task 6 待启动（bio_event_style，决策 4 默认走 AI 草稿直接 apply 模式）
...
当前位置：Task 4-5 已完成（决策 4 opportunistic 复核模式），Task 6 待启动（bio_event_style）。

Task 6+ 默认走决策 4 模式：AI 出草稿 → 直接 apply → 待 opportunistic 复核。
