...
> **状态**：M3 进行中 · Task 1-8 完成（person + work 全 AI 草稿入库，test 21/19/2）· Task 9 待启动（12 concept 创建）
...
| **M3（数据采集阶段 B 校对 + C 后来者旁注）** | **🔄 进行中（Task 9/17，10-17 待办）** | 见下表 |
...
## Task 4-8 残留待复核（opportunistic 模式延后）
...
- ✅ 跑 `npm test` 状态跳跃：21 / 19 PASS / **2 FAIL**（work 5 字段 RED→GREEN，剩 person + 12 concept）
- ✅ commit + push（Task 8 完成 + 此前 4 push 完）
- ⏭️ Task 9 待启动（12 核心 concept 创建——这是 schema 扩展，不是字段补充）
...
当前位置：Task 4-8 已完成（person + work 全 AI 草稿入库，test 21/19/2），Task 9 待启动（12 核心 concept 创建）。

Task 9 是 schema 扩展（新建 12 个 concept 节点），不像 4-8 是字段补充。仍按决策 4 模式：AI 出草稿 → 直接 apply → 待 opportunistic 复核。
PM 复核入口固定为 `docs/m3-validation/<scope>-checklist.md`（详见 anchor 决策 4 "PM 复核入口"，concept-checklist.md 在 Task 9 创建）。
