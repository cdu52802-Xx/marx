...
| **M3（数据采集阶段 B 校对 + C 后来者旁注）** | **🔄 进行中（Task 12/17，13-17 待办）** | 见下表 |
...
| 12 | 准备 SEP URL 清单 | ✅ DONE | 本轮 commit |
...
## Task 12 完成（2026-05-11）· 阶段 C SEP URL 清单 + 离线缓存目录

**目的**：主力机生成 PM 第三机要下载的完整 SEP URL 清单 + 命名规则 + .gitignore 排除规则（HTML 不入库，版权 + 体积考虑）。

**关键调整（A 模式对齐）**：

按 anchor 决策 1（A 模式 inline `successor_*` 字段），调整 plan 写法的 sep-urls.json schema：
- ❌ 去掉 `scholar_id`（A 模式 inline 不需要独立 entity ID，successor 嵌入 ConceptNode 数组）
- ❌ 去掉 `sep_concept_anchor`（plan 列的 #Alie / #TheoExpl 等 anchor 未验证，Task 14 AI parse 全文找段落更稳）
- ✅ 加 `successor_name_orig`（Task 14 直接写 SuccessorNote.successor_name_orig，省二次查 Wikidata）
- ✅ 字段从 `scholar_*` 改为 `successor_*`（跟 Node.ts L74-82 SuccessorNote 字段名一致）
- ✅ 加顶部 `$schema_note` / `$usage` / `$dedup_count` / `$dedup_url_list` 自描述 metadata（PM 阅读 + 给 Task 13 / 14 复用）

**实施**：
- ✅ 创建 [`scripts/sep-urls.json`](../scripts/sep-urls.json)（12 concept × 37 successor / 11 独立 URL）
  - 12 concept_id 跟 stage-b test L5-18 + nodes_skeleton.json 完全一致（unique IDs = 12）
  - 总 successor 数 = 37（异化 4 + 其他 11 个 × 3 = 37），落在 acceptance test [36, 60] 区间内
  - 11 独立 URL：Marx 主页 1 + SEP entries 6（lukacs / gramsci / althusser / habermas / luxemburg / marcuse）+ Wikipedia 4（lenin / jameson / harvey / balibar，SEP 无 entry 退化到 Wikipedia）
- ✅ 创建 [`scripts/sep-cache/.gitkeep`](../scripts/sep-cache/.gitkeep) 目录占位
- ✅ 修改 [`.gitignore`](../.gitignore) 末尾追加 `scripts/sep-cache/*.html` + `!scripts/sep-cache/.gitkeep`
- ✅ git check-ignore 验证：foo.html ignored / .gitkeep NOT ignored
- ✅ JSON sanity：concepts=12 / successors=37 / dedup=11 / unique IDs=12
- ✅ 跑 `npm test` 不变（26/23/3，没动 src/ 不影响测试）

**Successor 池分配概览**（PM 复核入口）：

| Successor（10）| 覆盖 concept 数 | concept 列表 |
|---|---|---|
| 葛兰西 | 7 | 历史唯物主义 / 阶级斗争 / 意识形态 / 经济基础与上层建筑 / 共产主义 / 革命 / 国家 |
| 阿尔都塞 | 8 | 异化 / 剩余价值 / 历史唯物主义 / 意识形态 / 生产方式 / 经济基础与上层建筑 / 劳动价值论 / 国家 |
| 卢森堡 | 5 | 剩余价值 / 阶级斗争 / 共产主义 / 革命 / 劳动价值论 |
| 列宁 | 4 | 阶级斗争 / 共产主义 / 革命 / 国家 |
| 哈维 | 3 | 剩余价值 / 生产方式 / 劳动价值论 |
| 卢卡奇 | 2 | 异化 / 商品拜物教 |
| 哈贝马斯 | 3 | 异化 / 意识形态 / 经济基础与上层建筑 |
| 马尔库塞 | 2 | 异化 / 商品拜物教 |
| 巴利巴尔 | 2 | 历史唯物主义 / 生产方式 |
| 詹姆逊 | 1 | 商品拜物教 |

**Plan vs 实际差异**：plan Step 1 列了 `scholar_id` / `sep_concept_anchor` 字段，跟最终落库的 SuccessorNote 字段（A 模式）不一致；本 task 按 anchor 决策 1 调整。Task 17 takeaway 一并修正 plan。

**待 PM 视情况复核**（opportunistic 模式）：
- 后来者池 10 人选择是否合理（阶段 C 之前可改 sep-urls.json）
- Lukács 名字带特殊字符（á 双 acute），中国大陆电脑显示是否正常
- 中文译名「巴利巴尔」vs「巴利巴」歧义（Étienne Balibar）

⏭️ Task 13 待启动（**PM 第三机操作**：浏览器另存为 11 个 HTML + 微信回传主力机 / 第三机能挂代理）
...
当前位置：Task 1-12 完成（含 Task 10 PM 反馈 + Task 11 阶段 C RED 立项 + Task 12 SEP URL 清单就绪）。Task 13 待启动（PM 第三机操作回传 11 个 SEP HTML）。
