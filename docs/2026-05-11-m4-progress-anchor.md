# Marx M4 续接锚点 · implementation 进行中（T0-T3 完成 / T4 待启动）

> **状态**（2026-05-12 T3 完成 update）：M4 implementation 进行中 · T0-T3 完成（含 12 concept hybrid 草稿入库 + PM 异步复核进行中）· **T4 待启动**（33 person × 3-5 quote 补采）· 测试 69/66/3
...
| **M4（claim-on-timeline 形态大重构）** | **🚧 implementation 进行中** · T0-T3 完成（hybrid 草稿入库）· T4 待启动 · npm test 69/66/3 | 见下表 + § 2026-05-12 update |
...
## 新窗口开场白模板（PM 复制粘贴用 · 2026-05-12 T3 完成 update）
...
2. docs/2026-05-11-m4-progress-anchor.md（M4 续接锚点 ⭐ 最重要）
3. docs/PRD.md § 4 V1 必做（含 ⚠ 实施状态注释）
4. specs/2026-05-11-m4-claim-timeline-design.md（M4 design doc · § 9.2 实施状态注释含 hybrid 模式）
5. plans/2026-05-11-marx-m4-claim-timeline.md § Task 4（T4 待启动）

当前位置（2026-05-12 T3 完成）：
- M4 implementation 进行中，T0-T3 完成（含 12 concept hybrid 草稿入 claims.json）
- 测试 69/66/3（3 RED 全是 M3 pre-existing baseline · stage-b alfred + stage-c × 2 · M3 Task 13-17 回头修，不属 M4）
- claims.json 已含: 19 Marx claims (Task 2) + 12 concept claims (Task 3) = 31 总
- subagent-driven 执行模式（PM 已选）

⭐ 关键策略（2026-05-12 PM 确认 hybrid 默认）:
- 数据采集 task 走 hybrid AI 草稿模式 = AI 草稿 → apply 入库 → commit + 启动下游 task
- PM 异步复核 checklist md（任意时间，不阻塞）
- "别让人工审核成为进度卡点" (PM 原话)
- 详见 memory feedback_hybrid_ai_draft_mode.md

M3 暂停状态：Task 1-12 完成 / Task 13-17 待 PM 第三机操作有窗口（M4 完成后回头）。

denizcemonduygu data.json: C:/Users/xuzequan/Desktop/denizcemonduygu-data.json
（988KB · 188 person · 2123 records · 8684 links · 已用 Marx 19 obs · T4/T5 仍需）

⚠ 已识别未排期 vision item:
- 地理图副视图（PRD § 4 V1 必做 silent drift，T6 PM checkpoint 时决定 A M5 专项 / B M4 中期 / C V2 推迟）

下一步：
- "go T4" → 派 subagent 跑 T4 (33 person × 3-5 quote 补采，⭐ M4 工时大头, 99-165 条 ClaimNode)
  hybrid 模式：AI 草稿 + 直接 apply 入库 + commit + 启动 T5，PM 复核异步
- "go T5" → claim → claim 关系借鉴 denizcemonduygu Marx 202 links（可与 T4 并行，T1 dep 已满足）
- "PM 异步复核 T3 concept 12 条" → 打开 docs/m4-validation/concept-12-claims-checklist.md
  改条目 + 跑 npm run m4:apply-md 同步（重点 3 条: communism / revolution / mode-of-production）
- "PM 异步复核 T2 Marx 19 条翻译" → 打开 docs/m4-validation/marx-19-claims-checklist.md
- "回 M3 做 Task 13" → 切回 M3 阶段 C（PM 第三机操作有窗口时）
```
